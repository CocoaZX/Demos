...
@implementation JingPaiPartTwoTableViewCell
...
- (void)layoutSubviews
{
    ...
    //self.statusBackView.backgroundColor = [UIColor orangeColor];
...
- (void)addButtons
{
    for (int i=0; i<self.statusArray.count; i++) {
        ...
        if ([type isEqualToString:@"0"]) {
            ...
            buttonView.imgView.contentMode = UIViewContentModeScaleAspectFill;
            ...
            [buttonView getTypeFourButtonViewWithImage:@"" WithIndex:[getSafeString(diction[@"jid"]) intValue]];
            buttonView.imgView.contentMode = UIViewContentModeScaleAspectFill;
        ...
        if (!isInTheArray) {
            ...
            {
                [self.viewsArray addObject:buttonView];
            }
        }
    }
...
- (void)deleteImage:(UIButton *)sender
{
    ...
    for (int i=0; i<self.statusArray.count; i++) {
        ...
        NSString *typeString = getSafeString(old[@"type"]);
    ...
    NSLog(@"%@",self.viewsArray);
    NSLog(@"%@",self.statusArray);
    NSLog(@"%@",self.picturesAlbumArray);

    if (removeIndex!=200) {
        [self.picturesAlbumArray removeObjectAtIndex:removeIndex-1];
...
float daojishi_jingpai = 0.0;

- (void)changeDaojishi
{
    daojishi_jingpai+=0.1;
    
    [UIView animateWithDuration:0.1 animations:^{
        self.daojishi.text = [NSString stringWithFormat:@"%.1f",daojishi_jingpai];
...
-(void)getSelectedPhoto:(NSMutableArray *)photos{
    ...
    NSMutableArray *statuArr = @[].mutableCopy;
    NSMutableArray *viewArr = @[].mutableCopy;
    
    if (self.statusArray.count>1&&self.viewsArray.count>1) {
        for (int i=0; i<self.statusArray.count; i++) {
            NSDictionary *diction = self.statusArray[i];
            NSString *type = getSafeString(diction[@"type"]);
            if ([type isEqualToString:@"0"]) {
                [statuArr addObject:diction];
...
//        [viewArr addObject:self.viewsArray[0]];
//        [viewArr addObject:self.viewsArray[1]];
    ...
    for (int i=0; i<self.picturesArray.count; i++) {
        ...
        [self.statusArray insertObject:@{@"type":@"3",@"image":image,@"jid":[NSString stringWithFormat:@"%d",(int)self.statusArray.count]}.mutableCopy atIndex:0];
...
- (void)uploadVideoImage
{
    ...
    NSDictionary *dict = [AFParamFormat formatPostUploadParamWithFileJingPaiShiPinFengMian:@"file"];
...
{
    NSArray *array = [[NSBundle mainBundle] loadNibNamed:@"JingPaiPartTwoTableViewCell" owner:self options:nil];
    JingPaiPartTwoTableViewCell *cell = array[0];
    cell.statusArray = @[@{@"type":@"0",@"image":@"",@"jid":@"0"}.mutableCopy,
                         @{@"type":@"1",@"image":@"",@"jid":@"1"}.mutableCopy].mutableCopy;
