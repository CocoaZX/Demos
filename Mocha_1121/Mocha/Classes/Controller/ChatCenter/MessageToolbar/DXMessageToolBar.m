...
@interface DXMessageToolBar()<UITextViewDelegate, DXFaceDelegate,UIActionSheetDelegate>
{
...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    //表情
    self.pictureButton = [[UIButton alloc] initWithFrame:CGRectMake(kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    self.pictureButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin;
    [self.pictureButton setImage:[UIImage imageNamed:@"camera"] forState:UIControlStateNormal];
    [self.pictureButton setImage:[UIImage imageNamed:@"camera"] forState:UIControlStateHighlighted];
    [self.pictureButton addTarget:self action:@selector(takePictureAction:) forControlEvents:UIControlEventTouchUpInside];
    self.pictureButton.tag = 6;
    //录制
    ...
    if (!self.moreView) {
        self.moreView = [[DXChatBarMoreView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 200) typw:ChatMoreTypeGroupChat];
    ...
    if (!self.faceView) {
        ...
        self.faceView.backgroundColor = COLOR(240, 242, 247);
...
//    [self.toolbarView addSubview:self.pictureButton];
...
{
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"拍照",@"从相册选", nil];
...
{
    if (buttonIndex==0) {
        [self.moreView takePicAction];

    }else if(buttonIndex==1)
    {
        [self.moreView photoAction];
