...
- (id)initWithMessageModel:(MessageModel *)model reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        //如果这是一条系统消息，是假借对方发的一条消息，此时的设置与普通消息并不同,并不需要头像等显示
        ...
        [self.headButton setFrame:CGRectMake(originX, CELLPADDING, HEAD_SIZE, HEAD_SIZE)];
...
- (void)setMessageModel:(MessageModel *)messageModel
{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",getSafeString(_messageModel.headImageURLString),getSafeString(jpg)];

    [self.headImageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:placeholderImage];
}
...
+ (NSString *)cellIdentifierForMessageModel:(MessageModel *)model
{
    ...
    switch (model.type) {
        ...
        {
            identifier = [identifier stringByAppendingString:@"YuePai"];
        }
            break;
        //打赏
        case eMessageBodyType_DaShang:
        {
            identifier = [identifier stringByAppendingString:@"DaShang"];
        }
            break;
        //新版本打赏：打赏金币
        case eMessageBodyType_GoldCoin:
        {
            identifier = [identifier stringByAppendingString:@"GoldCoin"];
        }
            break;
        //聊天界面中，聊天提示
        case eMessageBodyType_system:
        {
            identifier = [identifier stringByAppendingString:@"system"];
        }
            break;
        case eMessageBodyType_TaoXi:
        {
            identifier = [identifier stringByAppendingString:@"TaoXi"];
        }
        default:
            break;
    }
