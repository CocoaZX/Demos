...
//顶部按钮文字和图片的距离
@property(nonatomic,assign)CGFloat btnTxtPadding;
...
- (void)createMenuViewWithData:(NSArray *)data{
    ...
    for (int i = 0; i < num; i++) {
        ...
        //[btn setImage:[UIImage imageNamed:@"JuBaoArrow"] forState:UIControlStateNormal];
        
        //设置title和图片视图
        NSInteger fontSize = self.menuTitleFont ? self.menuTitleFont:12;
        CGFloat titleWidth = [SQCStringUtils getCustomWidthWithText:data[i] viewHeight:self.menuBaseHeight textSize:fontSize];
        CGFloat leftPadding = (kDeviceWidth/2 - titleWidth -_btnTxtPadding -11)/2;
        
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftPadding, 0, titleWidth, self.menuBaseHeight)];
        ...
        UIImageView *arrowImgView = [[UIImageView alloc] initWithFrame:CGRectMake(titleLabel.right +_btnTxtPadding, (self.menuBaseHeight -5)/2, 11, 5)];
        arrowImgView.tag = 601;
        arrowImgView.image = [UIImage imageNamed:@"JuBaoArrow"];
        [btn addSubview:arrowImgView];
        arrowImgView.transform = CGAffineTransformMakeRotation(M_PI);//旋转180度
    ...
    //水平方向分割线
    for (int i = 0; i < num; i++) {
        UILabel *lineLb = [[UILabel alloc]initWithFrame:CGRectMake((btnW+1)*i+btnW, self.menuBaseHeight/10*2, 1, self.menuBaseHeight/10*6)];
        lineLb.backgroundColor = [UIColor colorWithRed:219/255.0 green:219/255.0 blue:219/255.0 alpha:1];// 分割线颜色
        lineLb.backgroundColor = [CommonUtils colorFromHexString:@"#c4c4c4"];
        if (i == num - 1) {
            lineLb.hidden = YES;
        }
    ...
    [provinceBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
     provinceBtn.backgroundColor = [CommonUtils colorFromHexString:kLikeRedColor];
...
- (void)changeMenuDataWithIndex:(NSInteger)index{
    ...
    if ([self.allDataSource[index] count] ==1) {
        ...
        self.tableFirst.backgroundColor = [CommonUtils colorFromHexString:kLikeWhiteColor];
...
//    }
    ...
    UIImageView *imageView = [self getImageViewFromButtonIndex:self.lastSelectedIndex-100];
...
- (void)showFirstTableView:(UIButton *)btn{
    ...
    UILabel *titleLabel0 = [btn0 viewWithTag:600];
    UIButton *btn1 = [self.backView viewWithTag:tempTag +1];
    UILabel *titleLabel1 = [btn1 viewWithTag:600];

    if (btn == btn0) {
        titleLabel0.textColor = [CommonUtils colorFromHexString:kLikeRedColor];
        titleLabel1.textColor = [CommonUtils colorFromHexString:kLikeBlackColor];
//        [btn0 setTitleColor:[CommonUtils colorFromHexString:kLikeRedColor] forState:UIControlStateNormal];
//        [btn1 setTitleColor:[CommonUtils colorFromHexString:kLikeBlackColor] forState:UIControlStateNormal];
    }else{
        titleLabel1.textColor = [CommonUtils colorFromHexString:kLikeRedColor];
        titleLabel0.textColor = [CommonUtils colorFromHexString:kLikeBlackColor];
 //        [btn0 setTitleColor:[CommonUtils colorFromHexString:kLikeBlackColor] forState:UIControlStateNormal];
//        [btn1 setTitleColor:[CommonUtils colorFromHexString:kLikeRedColor] forState:UIControlStateNormal];
    }
...
//            }
            UIImageView *image = [self getImageViewFromButtonIndex:index-100];
            ...
            [UIView animateWithDuration:0.2 animations:^{
                ...
                if (index == 101) {
                    ...
                    provinceName =self.dataSourceFirst[_tempSelectedProvinceIndex];
                    
                    //是否打开二级列表，同时显示地址选择
                    if ([provinceName isEqualToString:@"不限" ]
                        ||[provinceName isEqualToString:@"香港"]
                        ||[provinceName isEqualToString:@"澳门"]){
                        //香港和澳门做特殊处理
                    }else{
                        //执行表视图的点击事件，显示表2
                        [self showTableTwo];
...
//                layer.transform = CATransform3DMakeRotation(M_PI*2, 0, 0, 1);
//            }
            UIImageView *imageView = [self getImageViewFromButtonIndex:index-100];
            imageView.transform = CGAffineTransformRotate(imageView.transform,M_PI);//旋转180度
            self.firstTableViewShow = NO;
            self.tableFirst.frame = CGRectMake(0, CGRectGetMaxY(self.backView.frame), self.tableViewWith, TableViewW);
            //隐藏表1
            [UIView animateWithDuration:0.2 animations:^{
...
- (void)showSecondTabelView:(BOOL)secondTableViewShow{
    CGFloat TableViewW = self.cellHeight*self.dataSourceSecond.count +40;
    CGFloat MaxHeigth = self.tableViewMaxHeight ? self.tableViewMaxHeight : tableViewMaxHeightDefalut;
    if (TableViewW > MaxHeigth) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.tableFirst) {
        ...
        if (!self.allData && !self.isNet) {
            ...
            cell1.textLabel.textColor = [CommonUtils colorFromHexString:kLikeBlackColor];            
         }else{
             if (indexPath.row == _tempSelectedProvinceIndex) {
                 cell1.backgroundColor = [UIColor whiteColor];
                 cell1.textLabel.textColor = [CommonUtils colorFromHexString:kLikeRedColor];
             }else{
    ...
    //表2
        ...
        if(indexPath.row == _tempcityCellIndex){
            cell2.textLabel.textColor = [CommonUtils colorFromHexString:kLikeRedColor];
        }else{
            cell2.textLabel.textColor = [CommonUtils colorFromHexString:kLikeBlackColor];
        }
        return cell2;
    }else if (tableView == self.tableThird){
...
//             layer.transform = CATransform3DMakeRotation(M_PI*2, 0, 0, 1);
//        }
        UIImageView *image = [self getImageViewFromButtonIndex:self.lastSelectedIndex-100];
        ...
        if (!weakSelf.dataSourceSecond && !weakSelf.dataSourceThird) {
            ...
            UILabel *titleLabel = [btn viewWithTag:600];
            titleLabel.text = weakSelf.dataSourceFirst[indexPath.row];
        ...
        if (weakSelf.dataSourceSecond && !weakSelf.dataSourceThird) {
            ...
            if (self.dataSourceSecond.count == 0) {
                NSString *provinceName = self.dataSourceFirst[_tempSelectedProvinceIndex];
                //[btn setTitle:provinceName forState:UIControlStateNormal];
                UILabel *titleLabel = [btn viewWithTag:600];
                titleLabel.text = provinceName;
                
                //记录选中的索引
                _tempProvinceCellIndex = _tempSelectedProvinceIndex;
            ...
            NSString *addressString = [NSString stringWithFormat:@"%@",city];
            //[btn setTitle:addressString forState:UIControlStateNormal];
            //UILabel *titleLabel = [btn viewWithTag:600];
            //titleLabel.text = addressString;

            [self resetBtnView:btn withTxt:addressString];
            //记录选中的索引
...
- (void)resetBtnView:(UIButton *)btn withTxt:(NSString *)addressString{
    //设置title和图片视图
    NSInteger fontSize = self.menuTitleFont ? self.menuTitleFont:12;
    CGFloat titleWidth = [SQCStringUtils getCustomWidthWithText:addressString viewHeight:self.menuBaseHeight textSize:fontSize];
    CGFloat leftPadding = (kDeviceWidth/2 - titleWidth -_btnTxtPadding -11)/2;
    
    UILabel *titleLabel = [btn viewWithTag:600];
    titleLabel.frame = CGRectMake(leftPadding, 0, titleWidth, self.menuBaseHeight);
    titleLabel.text = addressString;
    
    UIImageView *arrowImgView = [btn viewWithTag:601];
    arrowImgView.frame = CGRectMake(titleLabel.right +_btnTxtPadding, (self.menuBaseHeight -5)/2,11, 5);
     arrowImgView.image = [UIImage imageNamed:@"JuBaoArrow"];
}

- (UIImageView *)getImageViewFromButtonIndex:(NSInteger)btnIndex{
    NSInteger tempTag = self.menuButtonTag ? self.menuButtonTag :menuButtonDefalutTag;
    UIButton *btn = (id)[self viewWithTag:btnIndex+tempTag];
    UIImageView *imageView = [btn viewWithTag:601];
    return imageView;
}
...
//    }
    UIImageView *image = [self getImageViewFromButtonIndex:self.lastSelectedIndex-100];
    //image.transform = CGAffineTransformMakeRotation(M_PI*2);//旋转180度
    image.transform = CGAffineTransformRotate(image.transform, M_PI);
...
//        }
        UIImageView *image = [self getImageViewFromButtonIndex:index-100];
        //image.transform = CGAffineTransformMakeRotation(M_PI);//旋转180度
        image.transform = CGAffineTransformRotate(image.transform, M_PI);
