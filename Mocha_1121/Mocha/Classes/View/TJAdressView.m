...
-(void)animationForAdressView{
    ...
    if (self.cityDataArr.count <= 0) {
        [LeafNotification showInController:self.superVC withText:@"地区信息获取失败,请重新打开app"];
        return;
    }
    ...
    [UIView animateWithDuration:whiteViewTime*0.6 animations:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(whiteViewTime*0.6 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            _backButton.frame = CGRectMake(_backButton.frame.origin.x, _redView.frame.origin.y, redBtnW, redBtnH);
                ...
                _redBtn.backgroundColor = [UIColor colorForHex:kLikeRedColor];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:_provinceTableView]) {
        ...
        [_positionDict setValue:getSafeString(_provinceDataArr[indexPath.row][@"id"]) forKey:@"provinceID"];
        [_positionDict setValue:getSafeString(_provinceDataArr[indexPath.row][@"name"]) forKey:@"province"];
...
//        self.currentCityCell = cell;
        ...
        NSString *city = getSafeString(_cityDataArr[indexPath.row][@"name"]);
        if ([city isEqualToString:@"其他"]) {
            city = getSafeString(_positionDict[@"province"])?getSafeString(_positionDict[@"province"]):@"小模呵呵了";
        }
        
        city = [city stringByReplacingOccurrencesOfString:@"市" withString:@""];

        [self.positionDict setValue:city forKey:@"city"];
        [_positionDict setValue:getSafeString(_cityDataArr[indexPath.row][@"id"]) forKey:@"cityID"];
        
        _currentAdr = city;
        if (indexPath.row == 0) {
            
            _currentAdr = getSafeString(_cityDataArr[0][@"name"]);
            _currentAdr = [_currentAdr stringByReplacingOccurrencesOfString:@"市" withString:@""];
...
-(void)getLoctionInformation{
    ...
    NSDictionary *quanguoCity = @{@"id":@"0",@"name":@"全国"};
    NSArray *arr = [NSArray arrayWithObject:quanguoCity];
    NSDictionary *quanguo = @{@"id":@"0",@"name":@"全国",@"citys":arr};
    
    NSMutableArray *tempArr = [NSMutableArray array];
    [tempArr addObject:quanguo];
    //读取地区信息
    self.provinceDataArr = (NSMutableArray *)[ReadPlistFile readAreaArray];
    for (int i = 0; i<self.provinceDataArr.count; i++) {
        
        NSMutableArray *citys = [NSMutableArray array];
        NSString *provinceName = getSafeString(_provinceDataArr[i][@"name"]);
        NSDictionary *dict = @{@"id":@"0",@"name":provinceName};
        
        [citys addObject:dict];
        NSArray *cityArr = _provinceDataArr[i][@"citys"];
        [citys addObjectsFromArray:cityArr];
        
        NSDictionary *provinceDict = _provinceDataArr[i];
        NSMutableDictionary *proDict = [NSMutableDictionary dictionaryWithDictionary:provinceDict];

        [proDict setObject:citys forKey:@"citys"];
    
        [tempArr addObject:proDict];
    }
    self.provinceDataArr = tempArr;
    ...
    if (!dict) {
        ...
        NSString *indexStr = getSafeString(dict[@"index"]);
        NSInteger  index = indexStr.integerValue;
        
        _currentAdr = getSafeString(dict[@"city"]);
...
-(UIView *)redView{
    if (!_redView) {
        ...
        _redView.backgroundColor = [UIColor colorForHex:kLikeRedColor];
        _redView.alpha = 0.7;
...
-(UIButton *)backButton{
    if (!_backButton) {
        ...
        _backButton.backgroundColor = [UIColor colorForHex:kLikeRedColor];
